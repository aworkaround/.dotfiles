...
foreach ($App in $InstallPackages) {
    ...
    winget.exe install --id $App --silent --force --no-upgrade
    $i += 1
}
...
Write-Host "$i/$($InstallPackages.count) Packages and $($InstallFeatures.count) were installed successfully 🙌! Please launch a fresh PowerShell terminal 😊" -ForegroundColor Green